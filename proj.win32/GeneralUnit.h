...
public:
	...
	virtual void getCollided(int damage){}			//被鲲冲撞到的动画，需要有一个小位移，鲲如果在左边就往右边位移一下，鲲如果在右边就往左边位移一下（更高级的做法是沿着远离鲲的方向位移一小段）。播放一次，播放完进入正常活动状态或直接死掉，调一下die或dieOnCollision
